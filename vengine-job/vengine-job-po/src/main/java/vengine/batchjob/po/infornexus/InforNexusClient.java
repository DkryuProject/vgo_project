...
import vengine.batchjob.po.utils.PropertyUtil;
import vengine.batchjob.po.utils.Utils;
...
public class InforNexusClient {
	...
	public Optional<InforNexusOrderDetail> executeOrderDetail(Long uid, String status, Long documentId,
															  boolean amendment) throws IOException, InvalidKeyException, NoSuchAlgorithmException {

		File folder = new File(filePath + "/data/"+today+"/order");
		if(!folder.exists()){
			folder.mkdirs();
		}
		File file = new File(filePath + "/data/"+today+"/order/OrderDetail-" + uid + "-" + status + "-" + documentId + ".json");

		if (file.isFile()) {

			FileInputStream fis = new FileInputStream(file);
			BufferedReader reader = new BufferedReader(new InputStreamReader(fis));
			return Optional.of(new Gson().fromJson(reader, InforNexusOrderDetail.class));
		...
		if (amendment) {
			url = new URL("https://network.gtnexus.com/rest/3.1/OrderDetail/Amendment/" + documentId);
		} else {
			url = new URL("https://network.gtnexus.com/rest/3.1/OrderDetail/" + documentId);
		}
		...
		try (InputStream firstClone = new ByteArrayInputStream(baos.toByteArray());
			...
			InforNexusOrderDetail inforNexusOrderDetail = new Gson().fromJson(reader, InforNexusOrderDetail.class);
			return inforNexusOrderDetail == null ? Optional.empty() : Optional.of(inforNexusOrderDetail);
