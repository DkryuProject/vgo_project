...
const CbdMaterial = (props) => {
    ...
    const cbdInfoPost = useSelector((state) => state.cbdInfoReducer.post);
    const handleCbdInfoPost = useCallback(
        (payload) => dispatch(cbdInfoPostAsyncAction.request(payload)),
        [dispatch]
    );
    const handleCbdInfoPostInit = useCallback(
        () => dispatch(cbdInfoPostAsyncAction.initial()),
        [dispatch]
    );
    ...
    const handleSubmit = (values) => {
        ...
        return handleCbdInfoPost({ id: cbdId, type: type, data: values });
    };

    // 등록
    useEffect(() => {
        if (cbdInfoPost.error) {
            return handleNotification({
                type: 'error',
                message: 'Error',
                description: cbdInfoPost.error.message,
            });
        } else if (cbdInfoPost.data) {
            onLeftSplit();
            onShow({
                ...initialShow,
                materialWrite: {
        ...
        onLeftSplit,
        handleNotification,
        handleCbdInfoPostInit,
        handleCbdInfoGetLists,
    ]);

    return (
        <CbdMaterialWriteOutterWrap>
            <Drawer
                title=""
                width="500px"
                placement="right"
                        ...
                        >
                            <Form.Item name="usagePlace" label="usage">
                                {/* {FilterSelect({
                                    ...
                                    value: 'id',
                                    text: 'name',
                                    placeholder: 'Select Usage name',
                                    filterType: 'usage',
                                    data: companySearchLists,
                                    onData: () =>
                                        handleCompanySearchLists('usage'),
                                    // onAddItem: () => handleUsageAddItem(),
                                })} */}
                                <Input
                            ...
                            >
                                {type === 'fabric'
                                    ? FilterSelect({
                                          ...
                                          filter: (v) => v.name === 'yd',
                                          data: commonBasicGetUom,
                                          onData: () =>
                                              handleCommonBasicGetUom('length'),
                                      })
                                    : FilterSelect({
                                          _key: 'id',
                                          value: 'id',
...
const CbdMaterialWriteOutterWrap = styled.div`
    height: 100%;
    padding: 1rem;
    overflow: auto;

    #cbdMaterialWriteWrap {
        padding: 0.5rem;
