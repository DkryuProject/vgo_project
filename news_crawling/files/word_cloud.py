...
def wordcloud_from_text(language):
    ...
    try:
        sql_ko = """INSERT INTO crawling_word (created_at, word_nouns, country) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE word_nouns =%s, updated_at=%s;"""
        curs.execute(sql_ko, (str(now_date)[0:10], str(noun_list), str(language), str(noun_list), str(tmp_date)))
        print('complete ======= ' + str(tmp_date))
