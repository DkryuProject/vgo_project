...
class ArticleCrawler(object):
    def __init__(self):
        ...
        self.categories = {'FashionInsight':'News'}
    ...
    def crawling(self, category_name):
        ...
        url_format = f'http://www.fi.co.kr/main/list.asp?SectionStr={self.categories.get(category_name)}&daily='
        ...
        for url in list(reversed(target_urls)):
            ...
            temp_post = document.select('#wrap > div > div > ul > li')
            ...
            for line in list(reversed(temp_post)):
                # print('line ====> ' + str(line))
                url_temp = ''
                url_temp = line.a.get('href') if str(line.a.get('href')).startswith('http') else f'http://www.fi.co.kr/main/' + str(line.a.get('href'))
                # print('url_temp ====>\n' + url_temp)
            ...
            for content_url in post_urls:   # 기사 url
                ...
                try:
                    ...
                    # temp_now = str(temp_now)[0:10]

                    temp_now = datetime.datetime.strptime(str(self.date['start_year']) + "-" + str(self.date['start_month']) + "-" + str(
                    self.date['start_day']), '%Y-%m-%d')
                    # print('temp_now ====> ' + str(temp_now))
                    # time = re.findall('<span class="t11">(.*)</span>', request_content.text)[0]
                    time = document_content.find_all('meta', {'property':'og:start_time'})

                    # 기사 시간 초기화
                    text_time = ''
                    text_time = text_time + str(time[0].get('content')).replace('T', ' ') + ":00"
                    # print('text_time ====> ' + str(text_time))
                    temp_date = datetime.datetime.strptime(text_time, '%Y-%m-%d %H:%M:%S')
                    # print('temp_date ====> ' + str(temp_date)[0:10])
                    # print('temp_date ====> ' + str(temp_date) + ' - ' + str(temp_now))
                    ...
                    # 기사 본문 가져옴
                    tag_content = document_content.select('#wrap > div > div.cont_wrap > div.detail_wrap > div.d_cont > div')
                    # tag_content = re.findall('<p align="justify">(.*)</p>', tag_content.text)
                    # 뉴스 기사 본문 초기화
                    text_sentence = ''
                    ...
                    tag_reporter = document_content.select_one('#wrap > div > div.cont_wrap > div.detail_wrap > div.d_head > p.d_write').text


                    text_reporter = ''
                    text_reporter = text_reporter + str(tag_reporter.split('\xa0')[1]) + "(" + str(tag_reporter.split('\xa0')[2]) + ")"
                    # print('text_reporter =====> ' + text_reporter)
                    # 기사 언론사 가져옴
                    tag_company = document_content.find_all('meta', {'property':'og:site_name'})
...
if __name__ == "__main__":
    ...
    Crawler.set_category('FashionInsight')
