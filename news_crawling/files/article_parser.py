...
class ArticleParser(object):
    ...
    def clear_content_okfashion(cls, text):
        ...
        for i in range(0, len(blank_removed_content)):
            # reverse 된 기사 내용 중, ".다로 끝나는 경우 기사 내용이 끝난 것이기 때문에 기사 내용이 끝난 후의 광고, 기자 등의 정보는 다 지움
    ...
    def find_news_totalpage(cls, url):
        ...
        try:
            ...
            # headline_tag = document_content.select_one('#sections > section > nav > ul > li', {'class': 'current user-bg'})
            headline_tag = document_content.select('#sections > section > nav > ul > li')
            print('headline_tag ====> ' + headline_tag)
